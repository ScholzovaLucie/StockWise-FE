...
describe("OperationForm", () => {
  ...
  it("umožní výběr klienta", async () => {
    ...
    const klientCombobox = await screen.findByRole("combobox", {
      name: "Klient",
    });

    await act(async () => {
      fireEvent.mouseDown(klientCombobox);
    });

    const options = await waitFor(() => screen.getAllByRole("option"));

    fireEvent.click(options.find((opt) => opt.textContent === "Klient A"));

    expect(klientCombobox).toHaveTextContent("Klient A");
  });
