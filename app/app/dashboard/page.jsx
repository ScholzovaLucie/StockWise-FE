...
const DashboardPage = () => {
    ...
    getUserWidgets()
      .then((data) => {
        ...
        if (!widgetsData.length) {
          widgetsData = [
            { id: "overview", type: "overview", layout: getDefaultLayout("overview") },
            { id: "activeOperations", type: "activeOperations", layout: getDefaultLayout("activeOperations") },
            { id: "efficiency", type: "efficiency", layout: getDefaultLayout("efficiency") },
  ...
  const cleanLayout = (layouts) => {
    return Object.fromEntries(
      Object.entries(layouts).map(([key, layout]) => [
        key,
        layout.map(({ moved, isStatic, isDraggable, isResizable, ...rest }) => rest),
      ])
    );
  };
  ...
  const toggleEditMode = () => {
    ...
    saveUserWidgets({ widgets: updatedWidgets, layout: cleanedLayouts })
      .catch(console.error);
  };
  ...
  const removeWidget = (id) => {
    ...
    saveUserWidgets({ widgets: updatedWidgets, layout: updatedLayouts })
      .catch(console.error);
  };
  ...
  const addWidget = (widgetType) => {
    ...
    saveUserWidgets({ widgets: updatedWidgets, layout: cleanLayout(cleanedLayouts) })
      .catch(console.error);
  };
  ...
  const onLayoutChange = (currentLayout, allLayouts) => {
    if (editMode) {
      ...
      const updatedWidgets = widgets.map((widget) => ({
        ...widget,
        layout: {
          ...currentLayout.find((layout) => layout.i === widget.id) || widget.layout,
    ...
    <Box sx={{ p: 1 }}>
     <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
      ...
      <Tooltip title="Přepnout režim úprav">
        <IconButton color={editMode ? "primary" : "default"} onClick={toggleEditMode}>
          <EditIcon />
        </IconButton>
      </Tooltip>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <FormControl sx={{ minWidth: 200 }} size="small">
            ...
            <Select 
              ...
              defaultValue=""
            >
              <MenuItem value=""><em>Vyber widget</em></MenuItem>
              {Object.keys(widgetComponents)
                .filter((option) => !widgets.some((widget) => widget.type === option))
                .map((option) => (<MenuItem key={option} value={option}>{getWidgetTitle(option)}</MenuItem>))}
            </Select>
          </FormControl>
        </Box>
      )}
      ...
      >
        {widgets.map((widget) => {
          ...
          return (
            <div key={widget.id} data-grid={{ ...widget.layout, isDraggable: editMode, isResizable: editMode }}>
              <Paper sx={{ p: 2, height: "100%", position: "relative" }}>
                <Typography variant="h6">{getWidgetTitle(widget.type)}</Typography>
                {editMode && (
                  <IconButton sx={{ position: "absolute", top: 5, right: 5 }} onClick={() => removeWidget(widget.id)}>
                    <DeleteIcon />
                  </IconButton>
                )}
