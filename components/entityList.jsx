...
const EntityList = ({
  ...
  const handleSearch = useCallback(
    debounce(async (query) => {
      setLoading(true);
      try {
        const data = query ? await service.search(query) : await service.getAll(selectedClient);
        setEntities(data || []);
      } catch (error) {
        setError(error?.message || `Nepodařilo se vyhledat ${entityName}y.`);
      } finally {
        setLoading(false);
      }
    }, 300),
  ...
  useEffect(() => {
    handleSearch(search);
  }, [search, selectedClient, handleSearch]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setError(error?.message || `Nepodařilo se smazat ${entityName}.`);
    } finally {
      setLoading(false);
    }
  };
            ...
            columns={[
              ...columns,
              {
                ...
                renderCell: (params) => (
                  <>
                    <IconButton
                      onClick={() => router.push(`${viewPath}/${params.row.id}`)}
                      color="primary"
                    >
                      <VisibilityIcon />
                    </IconButton>
                    ...
                    {rowActions && rowActions(params)?.map((action, index) => (
